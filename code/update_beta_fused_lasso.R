update_beta_fused_lasso <- function(y,X,beta,tau,D,lambda,max_iter=1e2,tol=1e-4) {  
  ...
  for (i in 1:max_iter) {
    ...
    beta <- sol_fused_lasso(y, X, w, D, lambda)$beta
...
## QP to solve fused lasso problem
sol_fused_lasso <- function(y, X, w, D, lambda) {
