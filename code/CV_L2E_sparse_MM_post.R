CV_L2E_sparse_MM_post <- function(y, X, beta0, tau0, k, rhoSeq, beta_star){
  
  sol_final <- L2E_sparse_MM(y, X, beta=beta0, tau=tau0, kSeq = k, rhoSeq = rhoSeq, Show.Time = FALSE)
  
  ## Get active set to refine the raw L2E betahat
  true_active_set <- which(beta_star != 0)
  beta_final <- Proj_sparse(sol_final$Beta, k=k)
  activesetL2E <- which(beta_final != 0)
