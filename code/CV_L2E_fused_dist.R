...
#' 
#' \code{CV_L2E_fused_dist} performs k-fold cross-validation for robust fused regression under the L2 criterion with distance penalty
#' 
...
#' @export
#' 
CV_L2E_fused_dist <- function(y, X, beta0, tau0, D, kSeq, rhoSeq, nfolds=5, seed=1234, method="median",
  ...
  for (i in 1:nfolds) {
    ...
    res <- cv_fold_fused_dist(i, y, X, fold, cv.args, method=method)
    Loss[i, ] <- res
  }
...
cv_fold_fused_dist <- function(i, y, X, fold, cv.args, method="median") {
  cv.args$y <- y[fold!=i]
  cv.args$X <- X[fold!=i, , drop=FALSE]
  fit.i <- do.call("L2E_fused_dist", cv.args)
